module.exports = {
  ...
  PASS_ENEMY_FACTION_MESSAGES: [
    (username, target, factionName) => `@${username} passes the Black Diamond to @${target} of ${factionName}. Across faction lines. Voluntarily. The narrator is choosing not to comment!`,
    (username, target, factionName) => `The Black Diamond has been passed to @${target} of ${factionName}... by @${username}, their rival! This is apparently happening. +2 to ${factionName}.`,
    (username, target, factionName) => `@${username} hands the diamond to ${factionName}'s @${target}. The security system doesn't even know how to log this. +2 points to ${factionName}.`,
    (username, target, factionName) => `@${username} just passed to the enemy. This wasn't in the briefing. None of this was in the briefing!!`,
    (username, target, factionName) => `The narrator has covered heists in seventeen museums across four continents. @${username} passing to @${target} of ${factionName} is a first. +2 to ${factionName}.`,
    (username, target, factionName) => `@${username} to @${target}. Different factions. The chat has questions! The crew has questions! I HAVE QUESTIONS! +2 to ${factionName}.`,
    (username, target, factionName) => `@${username} what are you doing, @${username}? WHAT ARE YOU DOING?! @${target} of ${factionName} now has the diamond and +2 points. Congratulations to everyone involved!`,
    (username, target, factionName) => `@${username} passes to @${target}. The enemy. @${target}. Of ${factionName}. We're going to need a moment!!`,
    (username, target, factionName) => `Bold. Possibly genius. Or maybe not. @${username} hands the Black Diamond directly to ${factionName}'s @${target}. +2 points. The crew is filing paperwork!`,
    (username, target, factionName) => `@${username} pulls an Anakin. @${target} of ${factionName} is now holding the Black Diamond. This is where the fun begins. +2 points.`,
    (username, target, factionName) => `@${username}: "I am once again passing the Black Diamond to the enemy." @${target} of ${factionName} accepts. +2 points to ${factionName}.`,
    (username, target, factionName) => `@${username} has chosen chaos. Like a true Loki. The Black Diamond lands with @${target} of ${factionName}. +2 points. You're welcome, everyone!`,
    (username, target, factionName) => `Plot twist. M. Night Shyamalan could never! @${username} passes to ${factionName}'s @${target}. +2 points. The audience didn't see that coming.`,
    (username, target, factionName) => `@${username} said "it's not about the points, it's about sending a message." @${target} of ${factionName} received it. +2 points.`,
    (username, target, factionName) => `@${username} has gone full uno reverse on their own faction! @${target} of ${factionName} didn't even have to steal it. +2 points just landed in their lap!`
  ],
